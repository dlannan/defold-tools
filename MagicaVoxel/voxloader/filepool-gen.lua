...
local function makefile( fpath, fdata, binary )

	binary = binary or ""
	local fh = io.open(fpath, "w"..binary)
	fh:write(fdata)
	fh:close()
end
...
local function getfile( fpath, binary )

	binary = binary or ""
	local fh = io.open(fpath, "r"..binary)
	assert(fh)
...
local function makepoolfiles( count )
	...
	local tmpimg = getfile(meshgen.imagepath..meshgen.images.white, "b")
	local tmpimgblack = getfile(meshgen.imagepath..meshgen.images.black, "b")
	local tmpimgnorm = getfile(meshgen.imagepath..meshgen.images.norm, "b")
	...
	for i=1, count do 
		...
		-- Build a mesh file 
		local materialpath = meshgen.materialpath..meshgen.files.materialfile
		local newmdata = string.gsub(mdata, [[material: ("MATERIAL_FILE_PATH")]], "material: \"/"..materialpath.."\"",1)		
